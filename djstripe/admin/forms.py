...
def validate_public_url(value):
    """Validate that a URL is not a localhost or private IP address."""
    parsed = urlparse(value)
    hostname = parsed.hostname or ""

    if hostname in _LOOPBACK_HOSTNAMES:
        raise ValidationError(
            "This URL is not publicly accessible. "
            "For local development, use `manage.py stripe_listen` instead."
        )
...
class WebhookEndpointAdminCreateForm(WebhookEndpointAdminBaseForm):
    ...
    base_url = forms.URLField(
        required=True,
        validators=[validate_public_url],
        help_text=(
