...
def check_stripe_api_key(app_configs=None, **kwargs):
    ...
    if not djstripe_settings.STRIPE_SECRET_KEY:
        ...
        hint = "Add STRIPE_SECRET_KEY or STRIPE_TEST_SECRET_KEY and STRIPE_LIVE_SECRET_KEY to your settings."
        messages.append(checks.Critical(msg, hint=hint, id="djstripe.C001"))
    elif djstripe_settings.STRIPE_LIVE_MODE:
        if not djstripe_settings.STRIPE_SECRET_KEY.startswith("sk_live_"):
            msg = "Bad Stripe live API key."
            hint = 'The live API key should start with "sk_live_"'
            messages.append(checks.Critical(msg, hint=hint, id="djstripe.C002"))
    else:
        if not djstripe_settings.STRIPE_SECRET_KEY.startswith("sk_test_"):
            msg = "Bad Stripe test API key."
            hint = 'The test API key should start with "sk_test_"'
