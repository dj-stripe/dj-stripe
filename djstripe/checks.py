...
def check_stripe_api_key(app_configs=None, **kwargs):
    ...
    if not djstripe_settings.STRIPE_SECRET_KEY:
        ...
        if not djstripe_settings.STRIPE_SECRET_KEY.startswith(("sk_live_", "rk_live_")):
            msg = "Bad Stripe live API key."
            hint = 'STRIPE_SECRET_KEY or STRIPE_LIVE_SECRET_KEY should start with "sk_live_"'
            messages.append(checks.Critical(msg, hint=hint, id="djstripe.C002"))
    else:
        if not djstripe_settings.STRIPE_SECRET_KEY.startswith(("sk_test_", "rk_test_")):
            msg = "Bad Stripe test API key."
            hint = 'STRIPE_SECRET_KEY or STRIPE_TEST_SECRET_KEY should start with "sk_test_"'
