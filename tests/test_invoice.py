...
from tests import (FAKE_INVOICE, FAKE_CHARGE, FAKE_CUSTOMER,
                   FAKE_SUBSCRIPTION, FAKE_PLAN, FAKE_INVOICEITEM_II,
...
class InvoiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.customer = Customer.objects.create(
            subscriber=self.user, stripe_id=FAKE_CUSTOMER["id"], currency="usd")
    ...
    def test_invoice_with_subscription_invoice_items(
        ...
        item_id = "{invoice_id}-{sub_id}".format(
            invoice_id=invoice.stripe_id,
            sub_id=FAKE_SUBSCRIPTION["id"])  # composite key
    ...
    def test_upcoming_invoice(self, invoice_upcoming_mock,
                              subscription_retrieve_mock,
                              plan_retrieve_mock):
    ...
    @patch("stripe.Invoice.upcoming", side_effect=InvalidRequestError(
