	...
	def setUp(self):
		...
		for i in range(10):
			user = get_user_model().objects.create_user(
				username="patrick{0}".format(i), email="patrick{0}@example.com".format(i)
			)
...
class ChargeManagerTest(TestCase):
	def setUp(self):
		customer = Customer.objects.create(
			id="cus_XXXXXXX", livemode=False, account_balance=0, delinquent=False
		)
	...
	def test_get_paid_totals_for_april_2015(self):
		...
		self.assertEqual(
			decimal.Decimal("30.50"), paid_totals["total_amount"], "Total amount is not correct."
		)
		self.assertEqual(
