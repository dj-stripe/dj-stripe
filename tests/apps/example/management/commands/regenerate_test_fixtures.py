...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		self.fake_data_map = {
			...
			djstripe.models.Card: [tests.FAKE_CARD, tests.FAKE_CARD_II, tests.FAKE_CARD_V],
		...
		for n in range(2):
			...
			for model_class, old_objs in self.fake_data_map.items():
				readonly_fields = (
					common_readonly_fields + model_extra_readonly_fields.get(model_class, [])
				)
	...
	def init_fake_id_map(self):
		...
		for fake_customer in self.fake_data_map[djstripe.models.Customer]:
			try:
				...
				customer = djstripe.models.Customer(id=fake_customer["id"]).api_retrieve()
	...
	def unfake_json_ids(self, json_str):
		...
		for fake_id, actual_id in self.fake_id_map.items():
			...
			json_str = json_str.replace(
				f'"{FAKE_ID_METADATA_KEY}": "{actual_id}"', f'"{FAKE_ID_METADATA_KEY}": "{fake_id}"'
			)
	...
	def get_or_create_stripe_balance_transaction(self, old_obj):
		...
		if source.startswith("ch_"):
			...
			id_ = djstripe.models.StripeModel._id_from_data(charge["balance_transaction"])
	...
	def pre_process_subscription(self, create_obj):
		...
		if len(items):
			...
			for item in items:
				create_item = {
					k: v for k, v in item.items() if k in subscription_item_create_fields
				}

				create_item["plan"] = djstripe.models.StripeModel._id_from_data(create_item["plan"])
			...
			create_obj["plan"] = djstripe.models.StripeModel._id_from_data(create_obj["plan"])
	...
	def preserve_old_sideeffect_values(
		...
		for f, old_val in old_obj.items():
			...
			if isinstance(new_val, stripe.api_resources.ListObject):
				...
				f in sideeffect_fields and type(old_val) == type(new_val) and old_val != new_val
			):
