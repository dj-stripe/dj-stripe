...
class TestEarlyFraudWarning(CreateAccountMixin, AssertStripeFksMixin, TestCase):
    @patch(
    ...
    def test_sync_from_stripe_data(
        ...
        fake_early_fraud_warning_data = deepcopy(FAKE_EARLY_FRAUD_WARNING)

        fake_early_fraud_warning = EarlyFraudWarning.sync_from_stripe_data(
            fake_early_fraud_warning_data
        )

        self.assertEqual(
            fake_early_fraud_warning.id, fake_early_fraud_warning_data["id"]
        )

        self.assertEqual(
            fake_early_fraud_warning.charge.id,
            fake_early_fraud_warning_data["charge"]["id"],
        )
        self.assertEqual(
            fake_early_fraud_warning.payment_intent.id,
            fake_early_fraud_warning_data["payment_intent"]["id"],
        )
    ...
    def test___str__(
        ...
        self.assertEqual(
            str(fake_early_fraud_warning), "misc for $20.00 USD (Succeeded))"
        )
