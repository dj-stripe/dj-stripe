...
-   The `djstripe.webhooks` module has been removed. The `webhooks.handler`,
    `webhooks.handler_all` and `webhooks.call_handlers` functions are no longer
    available; use `djstripe.event_handlers.djstripe_receiver` instead. See the
    [migration guide](../usage/webhooks.md#migrating-from-the-legacy-djstripewebhooks-api)
    for examples. This change should have shipped with a deprecation cycle and
    did not — apologies for the disruption to projects upgrading from 2.8.
